...
def publish_data(
    ...
    if opt_res_latest is None:
        ...
        if opt_res_latest is None:
            data_path = input_data_dict["emhass_conf"]["data_path"] / filename
            if not os.path.isfile(data_path):
                logger.error("File not found error, run an optimization task first.")
                return

            # Load CSV
            opt_res_latest = pd.read_csv(data_path, index_col="timestamp")

            # Convert to datetime
            opt_res_latest.index = pd.to_datetime(opt_res_latest.index)

            # --- Ensure timezone-aware index ---------------------------------------
            tzname = input_data_dict["retrieve_hass_conf"]["time_zone"]

            # If index is naive (tz=None), localize it
            if opt_res_latest.index.tz is None:
                opt_res_latest.index = opt_res_latest.index.tz_localize(tzname)

            # Apply frequency guessing/correction
            opt_res_latest = utils.set_df_index_freq(opt_res_latest)

        # --- GENERATE TIMEZONE-AWARE "NOW" ------------------------------------------
        tzname = input_data_dict["retrieve_hass_conf"]["time_zone"]
        now_precise = pd.Timestamp.now(tzname).replace(second=0, microsecond=0)

        method = input_data_dict["retrieve_hass_conf"]["method_ts_round"]
